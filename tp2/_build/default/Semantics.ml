...
let (add, mem2) = (value_of_expr(_expr, mem) _env) in
match add with
  | ReferenceValue ref -> 
    (match (lookforMem ref mem2) with
    | NotFound -> (NullValue, mem2)
    | Found value -> (value, mem2))
...
let (v, mem2) = (value_of_expr(_valexpr, mem) _env) in
let (add, mem3) = (value_of_expr(_refexpr, mem2) _env) in
match add with
  | ErrorValue _ as result -> (result, mem3)
  | ReferenceValue ref -> (match (lookforMem ref mem) with
    | NotFound -> ((ErrorValue (UnknownReferenceError ref), mem3))
    | Found _ -> (NullValue, (ref, v)::mem3))
  | _ -> ((ErrorValue UndefinedExpressionError), mem3)
...
let (NullValue, mem2) = (value_of_expr(_left, mem) _env) in
(let (v, mem3) = (value_of_expr(_right, mem2) _env) in
(match v with
  | ErrorValue _ as result -> (result, mem3)
  | _ -> (v, mem3)))
...
let(condval, mem2) = (value_of_expr(_cond, mem), _env) in
match condval with
    | (BooleanValue true) ->
      let (NullValue, mem3) = (value_of_expr(_body, mem2) _env) in value_of_expr(WhileNode(_cond, _body), mem3)
...
let (v, mem2) = (value_of_expr(_expr, mem) _env) in 
match v with 
  | ReferenceValue ref -> 
    (match lookforMem ref mem2 with
    | NotFound -> (ReferenceValue ref, (ref, v)::mem2)
    | Found _ -> (ErrorValue (UnknownReferenceError ref), mem2)
    )
  | _ -> (ErrorValue TypeMismatchError, mem2)
;;
