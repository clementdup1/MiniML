...
let rec type_of_expr expr env =
  ...
  and
    ...
    let typeParam = newVariable () in
    let typeBody = type_of_expr _body ((_par, typeParam) :: _env) in
    FunctionType (typeParam, typeBody)
  and
    (* ...............A COMPLETER .......................................*)
    ruleCall _env _fct _par =
    let typeFct = type_of_expr _fct _env in
    let typePar = type_of_expr _par _env in
    match typeFct with
    | FunctionType (typeParam, typeResult) ->
      let _, unifyResult = unify typeParam typePar in
      if unifyResult then typeResult else ErrorType
    | _ -> ErrorType
  ...
  and
    (* ...............A COMPLETER .......................................*)
    ruleRef _env _expr =
    let typeExpr = type_of_expr _expr _env in
  ...
  and
    ...
    let typeLeft = type_of_expr _left _env in
    let typeRight = type_of_expr _right _env in
    match typeLeft with
    | ReferenceType t ->
        let _, unifyResult = unify t typeRight in
        if unifyResult then UnitType else ErrorType
    | _ -> ErrorType
  ...
  and
    ...
    let typeLeft = type_of_expr _left _env in
    let _, unifyResult = unify typeLeft UnitType in
    if unifyResult then
  ...
  and
    ...
    let typeCond = type_of_expr _cond _env in
    let typeBody = type_of_expr _body _env in
    let _, unifyCond = unify typeCond BooleanType in
    let _, unifyBody = unify typeBody UnitType in
    if unifyCond && unifyBody then UnitType else ErrorType
